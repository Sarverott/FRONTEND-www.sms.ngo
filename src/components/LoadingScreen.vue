...
async function getTextVersionHash(content) {
  return await window.crypto.subtle.digest('SHA-1', new TextEncoder().encode(content))
}
...
async function loadDoc() {
  try {
    ...
    websiteNotice.value = marked.parse(await response.text())
    console.log(websiteNotice)
  } catch (e) {}
}
</script>
<template>
